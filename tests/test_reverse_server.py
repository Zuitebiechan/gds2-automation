...
def test_handle_vci_connection_uses_proxy_local_session_state_when_snapshot_missing(
    ...
    async def _run() -> None:
        ...
        sample_frame = ProtocolEncoder.encode_local_live_data_sample(
            _proxy_local_engine_speed_sample(902.5),
            sequence=11,
        )
        reader = _FakeReader(auth_frame, sample_frame)
        writer = _FakeWriter(peername=("10.0.0.9", 9000))
    ...
    assert latest["session_id"] == "session-live"
    assert latest["live_data_active_at_receive"] is True
    assert latest["latest_sample"]["value"] == 902.5
    ...
    assert received["session_id"] == "session-live"
    assert received["live_data_active_at_receive"] is True


def test_handle_vci_connection_does_not_use_proxy_local_session_state_for_wrong_epoch(
    monkeypatch,
    tmp_path,
) -> None:
    async def _run() -> None:
        ...
        sample_frame = ProtocolEncoder.encode_local_live_data_sample(
            _proxy_local_engine_speed_sample(903.0),
            sequence=11,
        )
        reader = _FakeReader(auth_frame, sample_frame)
        writer = _FakeWriter(peername=("10.0.0.9", 9000))
...
def test_handle_proxy_connection_emits_read_msgs_cache_hit_metadata(monkeypatch, tmp_path) -> None:
    ...
    assert cache_decision["reason"] == "empty_cache_hit"
...
def test_handle_proxy_connection_merges_partial_prefetch_with_tunnel_data(monkeypatch, tmp_path) -> None:
    async def _run() -> None:
        ...
        server.vci_connected.set()
        server.vci_writer = _FakeWriter(peername=("10.0.0.9", 9000))
        server._connection_epoch = "epoch-prefetch-merge"
        server._next_sequence = lambda: 88
        prefetched = {
            "protocol_id": 6,
            "rx_status": 0,
        ...
        proxy_reader = _FakeReader(
            ProtocolEncoder.encode_read_msgs_req(77, num_msgs=3, timeout=0, sequence=21)
        )
        proxy_writer = _FakeWriter(peername=("127.0.0.1", 50002))
        ...
        assert (msg_type, sequence) == (MsgType.READ_MSGS_REQ, 88)
        assert ProtocolDecoder.decode_read_msgs_req(
            server.vci_writer.writes[0][HEADER_SIZE:]
        ) == (77, 2, 0)
        assert ProtocolDecoder.decode_read_msgs_rsp(proxy_writer.writes[0][HEADER_SIZE:]) == (
            0,
    ...
    assert cache_decision["reason"] == "prefetch_underfill_forwarded"
    assert cache_decision["prefetch_served_count"] == 1
    assert cache_decision["prefetch_underfill_count"] == 2
    assert response["reason"] == "prefetch_merge_tunnel_data"
    assert response["prefetch_merge_tunnel_message_count"] == 2
    assert response["message_count"] == 3


def test_prefetch_underfill_merge_caps_tunnel_messages_to_original_request() -> None:
    server = ReverseProxyServer(
        config=ProxyConfig.from_args(read_ahead_enabled=True, read_ahead_max_messages=4)
    )
    prefetched = {
    ...
    tunnel_messages = [
        {"protocol_id": 6, "rx_status": 0, "tx_flags": 0, "timestamp": 2, "data": b"\x62\x02"},
        {"protocol_id": 6, "rx_status": 0, "tx_flags": 0, "timestamp": 3, "data": b"\x62\x03"},
        {"protocol_id": 6, "rx_status": 0, "tx_flags": 0, "timestamp": 4, "data": b"\x62\x04"},
    ]
    server._prefetch_read_msgs.record_read_rsp_body(
        77,
        ProtocolEncoder.encode_read_msgs_rsp(0, [prefetched], sequence=0)[HEADER_SIZE:],
    )
    drain = server._prefetch_read_msgs.drain(77, 3)

    final_type, final_body, reason, fields, should_restore = (
        server._finalize_prefetch_underfill_response(
            drain,
            resp_type=MsgType.READ_MSGS_RSP,
            resp_body=ProtocolEncoder.encode_read_msgs_rsp(
                0,
                tunnel_messages,
                sequence=0,
            )[HEADER_SIZE:],
    ...
    assert final_type == MsgType.READ_MSGS_RSP
    assert reason == "prefetch_merge_tunnel_data"
    assert should_restore is False
    assert fields["prefetch_merge_tunnel_message_count"] == 3
    assert fields["prefetch_merge_tunnel_limited_message_count"] == 2
    assert ProtocolDecoder.decode_read_msgs_rsp(final_body) == (
        0,
        [prefetched, tunnel_messages[0], tunnel_messages[1]],
    )


def test_same_channel_prefetch_underfill_serializes_following_reads(monkeypatch, tmp_path) -> None:
    async def _run() -> None:
        monkeypatch.setenv("PROGRAMDATA", str(tmp_path))
        server = ReverseProxyServer(
            config=ProxyConfig.from_args(read_ahead_enabled=True, read_ahead_max_messages=4)
        ...
        server.vci_connected.set()
        server.vci_writer = _FakeWriter(peername=("10.0.0.9", 9000))
        server._connection_epoch = "epoch-prefetch-lock"
        sequences = iter([88, 89])
        ...
        async def _controlled_wait_for(awaitable, timeout):
            ...
            if isinstance(awaitable, asyncio.Future):
                ...
                return (
                    MsgType.READ_MSGS_RSP,
                    ProtocolEncoder.encode_read_msgs_rsp(BUFFER_EMPTY, [], sequence=0)[HEADER_SIZE:],
                    2.0,
                )
            return await awaitable

        monkeypatch.setattr("vci_proxy.reverse_server.asyncio.wait_for", _controlled_wait_for)

        first_writer = _FakeWriter(peername=("127.0.0.1", 50002))
        second_writer = _FakeWriter(peername=("127.0.0.1", 50003))
        first_task = asyncio.create_task(
            server._handle_proxy_connection(
                _FakeReader(ProtocolEncoder.encode_read_msgs_req(77, 2, 0, sequence=21)),
                first_writer,
            )
        )
        ...
        second_task = asyncio.create_task(
            server._handle_proxy_connection(
                _FakeReader(ProtocolEncoder.encode_read_msgs_req(77, 1, 0, sequence=22)),
                second_writer,
            )
        )
        ...
        assert ProtocolDecoder.decode_read_msgs_rsp(first_writer.writes[0][HEADER_SIZE:]) == (
            0,
            [prefetched, tunnel_message],
        )
        assert ProtocolDecoder.decode_read_msgs_rsp(second_writer.writes[0][HEADER_SIZE:]) == (
            BUFFER_EMPTY,
            [],
        )
...
def test_handle_proxy_connection_returns_prefetch_when_underfill_tunnel_empty(monkeypatch, tmp_path) -> None:
    async def _run() -> None:
        monkeypatch.setenv("PROGRAMDATA", str(tmp_path))
        server = ReverseProxyServer(
            config=ProxyConfig.from_args(read_ahead_enabled=True, read_ahead_max_messages=4)
        ...
        server.vci_connected.set()
        server.vci_writer = _FakeWriter(peername=("10.0.0.9", 9000))
        server._connection_epoch = "epoch-prefetch-empty"
        server._next_sequence = lambda: 89
        prefetched = {
            "protocol_id": 6,
            "rx_status": 0,
            "tx_flags": 0,
    ...
    assert response["reason"] == "prefetch_merge_tunnel_empty"
    assert response["prefetch_merge_tunnel_return_code"] == BUFFER_EMPTY
    assert response["return_code"] == 0
    assert response["message_count"] == 1


def test_handle_proxy_connection_restores_prefetch_when_underfill_tunnel_errors(monkeypatch, tmp_path) -> None:
    async def _run() -> None:
        monkeypatch.setenv("PROGRAMDATA", str(tmp_path))
        server = ReverseProxyServer(
            config=ProxyConfig.from_args(read_ahead_enabled=True, read_ahead_max_messages=4)
        ...
        server.vci_connected.set()
        server.vci_writer = _FakeWriter(peername=("10.0.0.9", 9000))
        server._connection_epoch = "epoch-prefetch-error"
        server._next_sequence = lambda: 90
        prefetched = {
            "protocol_id": 6,
            "rx_status": 0,
            "tx_flags": 0,
        ...
        async def _error_wait_for(awaitable, timeout):
            if isinstance(awaitable, asyncio.Future):
                return (
                    MsgType.READ_MSGS_RSP,
                    ProtocolEncoder.encode_read_msgs_rsp(7, [], sequence=0)[HEADER_SIZE:],
                    2.0,
                )
            return await awaitable

        monkeypatch.setattr("vci_proxy.reverse_server.asyncio.wait_for", _error_wait_for)
        ...
        restored = server._prefetch_read_msgs.try_serve(77, 1, 22)
        assert restored is not None
        assert ProtocolDecoder.decode_read_msgs_rsp(restored[HEADER_SIZE:]) == (0, [prefetched])
    ...
    assert response["reason"] == "prefetch_underfill_tunnel_error"
    assert response["prefetch_restored_count"] == 1
    assert response["return_code"] == 7
...
def test_build_tunnel_request_deepens_read_collect_after_fifo_exhaustion() -> None:
    server = ReverseProxyServer(
        config=ProxyConfig.from_args(
            read_ahead_enabled=True,
            read_ahead_transaction_enabled=True,
    ...
    assert request.max_messages == 16
    assert transaction_fields == {
        "read_collect_budget_reason": "after_prefetch_partial_hit",
        "read_collect_budget_deepened": True,
        "read_collect_collect_window_ms": 40,
        "read_collect_max_reads": 6,
        "read_collect_read_timeout_ms": 0,
        "read_collect_max_messages": 16,
    }


def test_build_tunnel_request_keeps_read_collect_light_after_confirmed_empty() -> None:
    server = ReverseProxyServer(
        config=ProxyConfig.from_args(
            read_ahead_enabled=True,
            read_ahead_transaction_enabled=True,
    ...
    encoded, _fwd_type, _fwd_body, reason = server._build_tunnel_request(
        MsgType.READ_MSGS_REQ,
        read_body,
        sequence=77,
        transaction_fields=transaction_fields,
    ...
    assert reason == "read_collect_transaction"
    assert request.max_reads == 3
    assert transaction_fields["read_collect_budget_reason"] == "standard_read_tail"
    assert transaction_fields["read_collect_budget_deepened"] is False


def test_build_tunnel_request_deepens_after_confirmed_empty_following_fifo_drain() -> None:
    server = ReverseProxyServer(
        config=ProxyConfig.from_args(
            read_ahead_enabled=True,
            read_ahead_transaction_enabled=True,
    ...
    assert reason == "read_collect_transaction"
    assert request.max_reads == 6
    assert transaction_fields["read_collect_budget_reason"] == (
        "after_confirmed_empty_following_prefetch_drain"
    )
...
def test_read_collect_budget_keeps_confirmed_empty_after_fifo_drain_timeboxed() -> None:
    ...
    budget = server._read_collect_transaction_budget(44, now=time.monotonic() + 0.2)

    assert budget.reason == "standard_read_tail"
    assert budget.deepened is False
    assert budget.max_reads == 3


def test_read_collect_budget_ignores_prefetch_history_before_confirmed_empty() -> None:
    server = ReverseProxyServer(
        config=ProxyConfig.from_args(
            read_ahead_enabled=True,
            read_ahead_transaction_enabled=True,
    ...
    budget = server._read_collect_transaction_budget(44)
...
def test_shadow_plan_filters_uds_dids_with_include_and_exclude(monkeypatch, tmp_path) -> None:
    monkeypatch.setenv("PROGRAMDATA", str(tmp_path))
    ...
    async def _run() -> None:
        ...
        server._connection_epoch = "epoch-did-filter"
        server._vci_sweep_shadow_supported = True
        sent_plans: list[SweepPlanStartRequest] = []
        ...
        _observe_pair(b"\x00\x00\x07\xe0\x22\x00\x0c", b"\x00\x00\x07\xe8\x62\x00\x0c\x12\x34", 71)
        _observe_pair(b"\x00\x00\x07\xe0\x22\x00\x31", b"\x00\x00\x07\xe8\x62\x00\x31\x56\x78", 73)
        _observe_pair(b"\x00\x00\x07\xe0\x22\x00\x42", b"\x00\x00\x07\xe8\x62\x00\x42\x9a\xbc", 75)
        await asyncio.sleep(0)

        assert sent_plans
        assert len(sent_plans[0].requests) == 1
        request = sent_plans[0].requests[0]
        channel_id, messages, _timeout = ProtocolDecoder.decode_write_msgs_req(
            request.write_req_body
        )
        assert channel_id == 44
        assert messages[0]["data"] == b"\x00\x00\x07\xe0\x22\x00\x0c"

    asyncio.run(_run())


def test_shadow_plan_uses_real_read_request_shape(monkeypatch, tmp_path) -> None:
    monkeypatch.setenv("PROGRAMDATA", str(tmp_path))
    ...
    async def _run() -> None:
        ...
        server._connection_epoch = "epoch-read-shape"
        server._vci_sweep_shadow_supported = True
        sent_plans: list[SweepPlanStartRequest] = []
        ...
        write_body = ProtocolEncoder.encode_write_msgs_req(
            44,
            [{"protocol_id": 6, "rx_status": 0, "tx_flags": 64, "timestamp": 1, "data": b"\x00\x00\x07\xe0\x22\x00\x0c"}],
            timeout=25,
        )[HEADER_SIZE:]
        ...
        read_rsp_body = ProtocolEncoder.encode_read_msgs_rsp(
            0,
            [{"protocol_id": 6, "rx_status": 0, "tx_flags": 0, "timestamp": 2, "data": b"\x00\x00\x07\xe8\x62\x00\x0c\x12\x34"}],
        )[HEADER_SIZE:]
        ...
        assert sent_plans[0].requests[0].read_timeout_ms == 0

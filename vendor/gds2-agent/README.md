...
1. Window title keywords ("Data Display", "DTC")
2. Unique button presence ("Create Report" → DATA_DISPLAY)
3. List content patterns ("Module Diagnostics" → DIAGNOSTICS_MENU)
4. Button combination + list emptiness heuristics

### Modified File: `CommandMonitor.java`
Added `"get_page_id"` case to the `executeCommand()` switch statement.
...
| `module_list` | GDS2Page.MODULE_LIST | Items with [K20] patterns |
